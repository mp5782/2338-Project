library(lme4)
library(lattice)
data_1 <- read.csv("https://raw.githubusercontent.com/mp5782/2338-Project/main/data/agg_sep_indi_3_4_mobility.csv")
data_2 <- subset(data_1,state!='India')
lmer_mobility <- lmer(Mobility ~ days +
                        Phase_indi_.1.0 +
                        Phase_indi_0.0 +
                        Phase_indi_1.0 +
                        Phase_indi_2.0 +
                        Phase_indi_3.0 +
                        Phase_indi_4.0 + Phase_indi_5.0 +
                        (1| state),
                      data = data_2)
...
for (s in states){
  ...
  #create phase indicator lines for every lockdown date
  #it looks like the "text" labels aren't working??
...
##########     SCRAP CODE, TO BE DELETED     ##########     
#lm / lmer of national level
data_3 <- subset(data_1,state=='India')


lmer_national <- lmer(Mobility ~ days +
                        Phase_indi_.1.0 +
                        Phase_indi_0.0 +
                        Phase_indi_1.0 +
                        Phase_indi_2.0 +
